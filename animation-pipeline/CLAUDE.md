...
| `pipeline/kit.py` | the kit template sheet: `template` writes an A4 with fiducials, red joint dots and NOTHING else in the draw areas (no ghost guides — Aaron draws on the image on his phone with a pen, and printed marks show through digital paint); `ingest` straightens the filled sheet off the corner squares and writes the whole character folder (parts, rig.json with pivots+face, assembled body.png). `tools/kit_template.pdf` is the committed copy Aaron draws on — regenerate it after layout changes and re-run the blank-sheet + composited round trip |
