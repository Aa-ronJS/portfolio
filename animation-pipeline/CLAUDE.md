...
| `pipeline/kit.py` | the kit template sheet: `template` writes an A4 with fiducials, red joint dots and NOTHING else in the draw areas (no ghost guides — Aaron draws on the image on his phone with a pen, and printed marks show through digital paint); `ingest` straightens the filled sheet off the corner squares and writes the whole character folder (parts, rig.json with pivots+face, assembled body.png). `tools/kit_template.png` is the committed copy Aaron draws on (`.pdf` alongside for printing) — regenerate both after layout changes and re-run the blank-sheet + composited round trip |
...
No ghosts at all (same day, the real workflow surfaces): Aaron does
not print the sheet — he opens it on his phone and draws on it with
an e-pen, so EVERY printed mark inside a draw area sits under his
strokes and shows through semi-transparent paint. The grey ghosts
from the previous round are gone entirely; the template now carries
only functional marks: borders, labels, dots, fiducials. The border
was darkened to stay visible on screen (185), which exposed a latent
trap: a border dark enough to read as ink forms a CLOSED LOOP around
every cell, and clean()'s fill-holes floods each box interior solid —
a blank cell ingests as a fully-opaque drawing. Borders are now
erased BY POSITION before cleaning (like the dots), so their printed
colour is irrelevant. Validation bar used and worth keeping: ingest
the BLANK template (must error "torso box looks empty" — proves no
...
`tools/kit_template.png` is the file Aaron actually draws on.
