...
(function () {
  ...
  // Test drive: while it is on, nothing reaches a customer. Every text and email is re-addressed to the
  // painter's own number and inbox, with a line at the top saying where it would have gone. The relay, Twilio,
  // Resend, the scheduling and the reply routing are all still the real ones -- only the destination changes,
  // because a test that fakes the sending does not test the sending.
  function divert(payload) {
    var S = window.QCStore ? QCStore.load() : {}, t = S.testdrive || {};
    if (!t.on || t.divert === false) return payload;
    if (!/^(send|schedule)$/.test(payload.action)) return payload;
    var mine = payload.channel === 'sms' ? String((S.details && S.details.phone) || '').trim() : String((S.details && S.details.email) || '').trim();
    if (!mine) return payload;                       // nowhere of his own to send it: leave it alone and let it fail loudly
    var was = String(payload.to || '(nobody)');
    if (was === mine) return payload;
    payload.to = mine;
    var tag = 'TEST DRIVE. This would have gone to ' + was + '.';
    if (payload.body != null) payload.body = tag + '\n\n' + payload.body;
    if (payload.subject != null) payload.subject = '[TEST] ' + payload.subject;
    payload.diverted_from = was;
    return payload;
  }
