...
(function () {
  ...
  function signoff(st) { var d = (st || S).details, name = String(d.owner_name || '').trim(); if (d.first_name_signoff !== false && name) name = name.split(/\s+/)[0]; var biz = String(d.trading_name || '').trim(); return (d.sign_off || 'Cheers') + (name ? ', ' + name : biz ? ', ' + biz : ''); }
  ...
  function viewHome() {
    ...
    if (hs.hosted === true) {
      ...
      else if (hs.hosted_cancelled === true) html += '<div class="card" id="hostedend"><p class="hint"><b>Cancelled.</b> ' + (hTo ? 'The chasing runs to ' + esc(hTo) + ' and stops after that.' : 'The chasing stops at the end of what you have paid for.') + ' Change your mind in <a href="#/settings">Set-up</a>.</p></div>';
  ...
  function whereItsAt(job) {
    ...
    return '<div class="card"><p>' + lines.join(' ') + '</p><div class="row">' + btns.map(function (b) { return '<a class="btn sm ' + b[2] + '" href="' + b[0] + '">' + b[1] + '</a>'; }).join('') + '</div>' + (st === 'quoted' && job.sent_date ? '<p class="hint">' + who + ' has the sent quote until you send again.</p>' : locked && paintsHere() && !fromBook(job) ? '<p class="hint">Locked. Revise quote reopens it.</p>' : '') + '</div>';
  }
  ...
  // the picture of his own quote, kept when it was brought in by photo or PDF
  function quotePic(job) { var ph = (job && job.photos || []).filter(function (x) { return x && /^data:image\//.test(String(x.data || '')); }); var q = ph.filter(function (x) { return /quote/i.test(String(x.caption || '')); })[0] || (fromBook(job) ? ph[0] : null); return q ? q.data : ''; }
  function markWon(job, after) {
    job.status = 'accepted'; job.acceptance = { date: QCStore.today(), how: job.sent_how === 'email' ? 'email' : 'text', note: '', by: job.client.first_name || job.client.name || '' }; save();
    var dinv = draftDepositInvoice(job);
    cancelQuoteFollowUps(job).then(function () { after(); toast(dinv ? 'Accepted. Deposit invoice ' + dinv.no + ' for ' + amtS(dinv.total) + ' is written.' : (S.details.gst && !S.details.abn && depositFor(job, (job.quote || {}).total || 0).amount > 0 ? 'Accepted. Add your ABN in Set-up and the deposit invoice can be written.' : 'Accepted. Note how they said yes.')); });
  }
  function markLost(job, after) { job.status = 'declined'; save(); cancelQuoteFollowUps(job).then(function () { after(); }); }
  function viewJob(job) {
    ...
    var html = '<a class="hint" href="#/">&larr; Jobs</a><div class="row between"><h1>' + esc(job.no_number ? (job.client.name || 'Job') : job.quote_no) + '</h1>' + statusPill(job) + '</div>';
    html += whereItsAt(job);
    // Won or lost, right where he looks: on every trade's job, not only on the quote page
    // a figure brought in from his own quote can be put right here, if he typed it wrong
    if (job.status === 'quoted' && fromBook(job)) html += '<div class="card"><label class="f">Quote total<span>' + (S.details.gst !== false ? 'with GST' : 'the total') + '</span><input type="text" id="jamount" inputmode="decimal" value="' + esc(String(job.quote.total)) + '" style="font-size:1.3rem"></label></div>';
    if (job.status === 'quoted' && !paintsHere()) html += '<div class="row tiles wonlost"><button class="btn tile" id="jwon"' + QCPics.says('Won: they said yes') + '>' + QCPics.tile('yes', 'Won') + '</button><button class="btn ghost tile" id="jlost"' + QCPics.says('Lost: they said no') + '>' + QCPics.tile('no', 'Lost') + '</button></div>';
    ...
    html += '<div class="card"><h2>Client</h2><div class="g2"><label class="f">Client type<select data-bind="client.type" id="ctype"><option value="homeowner">Homeowner</option><option value="agent">Agent</option><option value="strata">Strata</option><option value="builder">Builder</option><option value="commercial">Commercial</option></select></label><label class="f">Name<span>person or business</span><input type="text" data-bind="client.name" autocomplete="off"></label><label class="f">Contact first name<span>for texts</span><input type="text" data-bind="client.first_name" autocomplete="off"></label><label class="f">Mobile<input type="tel" data-bind="client.phone"></label></div>' + (typeLine ? '<p class="hint" id="ctypeline">' + esc(typeLine) + '</p>' : '') +
      ...
      '<label class="f">Description<span>the client sees this</span><input type="text" data-bind="summary" placeholder="' + (paintsHere() ? 'Repaint lounge and hall' : 'Switchboard upgrade') + '"></label><label class="f">Notes for the client<textarea data-bind="notes_client" rows="2" placeholder="' + (paintsHere() ? 'Colours, access, parking, furniture' : 'Access, parking, pets') + '"></textarea></label></div>';
    html += siteCard(job);
    html += '<div class="card ponly"><div class="row between"><h2>Rooms and outside areas</h2><div class="row"><button class="btn sm" data-add="interior">+ Room</button><button class="btn ghost sm" data-add="exterior">+ Exterior</button></div></div>';
    ...
    html += '<div class="card ponly"><div class="row between"><h2>Extras</h2><button class="btn ghost sm" id="addextra">+ Line</button></div><p class="hint">Not in a room.</p><div id="extras"></div>' +
    ...
    html += '<div class="card"><h2>Deposit and terms</h2><div class="g3"><label class="f">Deposit %<span>blank = ' + dp + '%' + (biz ? ', business terms' : '') + '</span><input type="number" data-bind="deposit_pct" data-refresh="1" min="0" max="100" step="0.5" placeholder="' + dp + '"></label><label class="f">Payment days<span>blank = ' + bd + (biz ? ', business terms' : '') + '</span><input type="number" data-bind="balance_days" data-refresh="1" min="0" placeholder="' + bd + '"></label><label class="f">Deposit<span id="depinfo">' + esc(depInfo(priced, job)) + '</span></label></div>' +
      '<div class="row"><label class="btn ghost sm"><input type="checkbox" data-bind="auto_follow_ups"> ' + esc(fuLabel()) + '</label><label class="btn ghost sm ponly"><input type="checkbox" data-bind="client_paint" data-refresh="1"> Client supplies paint</label><label class="btn ghost sm"><input type="checkbox" data-bind="hold.on" id="holdon"> Hold reminders</label></div><p class="hint">' + esc(fuHint()) + '</p>' +
    ...
    html += '<div class="card ponly"><div class="row between"><h2>Colours</h2><button class="btn ghost sm" id="addcol">+ Colour</button></div><div id="cols"></div></div>';
    ...
    html += '<div class="card ponly"><div class="row between"><div><span class="hint">Running total' + (S.details.gst ? ' inc GST' : '') + '</span><h2 id="runtotal">' + money(priced.total) + '</h2></div><a class="btn tape" href="#/job/' + job.id + '/quote">' + (job.quote ? 'Open the quote' : 'Build the quote') + '</a></div>' + (priced.confirm.length ? '<p class="confirm">' + priced.confirm.length + ' line' + (priced.confirm.length > 1 ? 's' : '') + ' to confirm</p>' : '') + (priced.options.length ? '<p class="hint">' + priced.options.length + ' option' + (priced.options.length > 1 ? 's' : '') + ' priced outside the total.</p>' : '') + '<div class="row"><button class="btn ghost sm" id="showcost">Show my costs</button></div><p class="hint" id="costline" hidden>' + costLine(priced) + '</p></div>';
    ...
    $app.innerHTML = html;
    var ja = document.getElementById('jamount'); if (ja) ja.addEventListener('change', function () {
      var v = QCIngest.parseMoney(ja.value), q = job.quote; if (!(v > 0)) { ja.value = String(q.total); toast('Type the amount, like 2450.'); return; }
      if (Math.abs(v - q.total) < 0.005) return;
      var gstOn = S.details.gst !== false, sub = gstOn ? r2(v / 1.1) : r2(v);
      q.total = r2(v); q.subtotal = sub; q.gst = r2(v - sub); q.deposit = null; job.manual_total = sub;
      (q.lines || []).forEach(function (l) { if (l.key === 'room_price') { l.rate = sub; l.base_rate = sub; l.amount = sub; } });
      bookQuote(job); save();
      // the nudges already booked carry the old figure: take them back and book them again
      cancelQuoteFollowUps(job).then(function () { return chaseable(job) && (QCMsg.ready('sms') || QCMsg.ready('email')) ? queueLoadedFollowUps([job]) : null; }).catch(function () {}).then(function () { toast('Now ' + money(v) + '. The follow-ups say so too.'); viewJob(job); });
    });
    var jw = document.getElementById('jwon'); if (jw) jw.addEventListener('click', once(function () { markWon(job, function () { viewJob(job); }); }));
    var jl = document.getElementById('jlost'); if (jl) jl.addEventListener('click', once(function () { markLost(job, function () { viewJob(job); }); }));
  ...
  function bookBox(job) {
    ...
    html += '<div id="bookbox" class="scalebox" ' + (editing ? '' : 'hidden') + '><b id="bk_title">' + (editing ? 'Change the booking' : 'Book these days') + '</b><div class="g3"><label class="f">Start<span id="bk_nice">' + esc(niceDay(editing ? b.start : QCStore.addDays(QCStore.today(), 7))) + '</span><input type="date" id="bk_start" value="' + esc(editing ? b.start : '') + '"></label><label class="f">Days<input type="number" id="bk_days" min="1" value="' + (editing ? b.days : days) + '"></label><label class="f">Start time<span>hour, e.g. 7</span><input type="number" id="bk_hour" min="5" max="12" value="' + esc(editing && b.hour ? b.hour : S.booking.start_hour) + '"></label></div>' +
      '<p>' + (job.booking_days >= 1 ? 'Days as you set them last time.' : 'The app reckons ' + days + ' day' + (days === 1 ? '' : 's') + ' at ' + hoursPerDay() + ' hours a day with ' + crew + (paintsHere() ? ' painter' + (crew === 1 ? '' : 's') : (crew === 1 ? ' person' : ' people')) + '. Change it if you know better.') + '</p>' +
  ...
  function viewQuote(job, keep) {
    ...
    var acc = document.getElementById('accepted'); if (acc) acc.addEventListener('click', function () { markWon(job, function () { viewQuote(job, true); }); });
    ...
    var dec = document.getElementById('declined'); if (dec) dec.addEventListener('click', function () { markLost(job, function () { viewQuote(job, true); }); });
  ...
  function viewInvoice(job, keep, qs) {
    ...
    var varInc = r2(unInv.reduce(function (s2, v) { return s2 + v.amount; }, 0) * (gstOn ? 1.1 : 1)), jobTotal = r2(q.total + varInc), billed = r2(issued.reduce(function (s2, i) { return s2 + i.total; }, 0)), leftToBill = hasFinal ? 0 : Math.max(0, r2(jobTotal - billed - (draft ? draft.total : 0)));   // a written deposit is already counted in what is owing
  ...
  function quoteEmailText(job, priced) {
    ...
    return greetLine(job) + '\n\nQuote ' + qno + ' for ' + desc + ' is attached: ' + money(v.total) + (v.gst ? ' inc GST' : '') + ', valid until ' + QCPdf.fmtDate(valid) + '.' + (q.version > 1 ? ' It replaces the earlier ' + job.quote_no + '.' : '') + (paintsHere() && !fromBook(job) ? ' The quote lists what is and is not included; please read it before accepting.' : '') + '\n\n' + accept + '\n\n' + signLine() + (det.trading_name ? '\n' + det.trading_name : '') + (det.phone ? ' · ' + det.phone : '');
  }
  ...
  function bookJob(job, start, days, hour) {
    ...
    var e = { uid: job.id + '-book-' + start, summary: (paintsHere() ? 'Painting: ' : 'Job: ') + (job.client.name || job.quote_no) + (job.summary ? ' - ' + job.summary : ''), description: 'Quote ' + job.quote_no + ', ' + money(job.quote ? job.quote.total : 0) + '. ' + (job.client.phone || '') + ' ' + (job.client.email || '') + '\n' + (job.notes || ''), location: job.client.address || '', start: start, end: endExcl };
  ...
  function viewChase() {
    ...
    if (owed > 0.004) html += '<div class="card overdue"><div class="row between"><span>Overdue</span><h2>' + money(owed) + '</h2></div></div>';
    ...
    if (groups.length) html += '<div class="card"><h3>Scheduled</h3><p class="hint">These go by themselves.</p>' + groups.map(function (g) {
      return '<div class="sgroup"><b>' + esc(g.job.client.name || g.job.quote_no) + '</b> <span class="hint">' + esc(g.job.no_number ? '' : g.job.quote_no) + (g.job.hold && g.job.hold.on ? ' · on hold' : '') + '</span>' +
        ...
        g.wait.map(function (w) { var k = flat.push({ job: g.job, w: w }) - 1; return '<div class="sitem"><div class="row between"><span><b>' + QCPdf.fmtDate(w.day) + '</b> by ' + (w.channel === 'sms' ? 'text' : 'email') + ' <span class="hint">' + esc(refLabel(w.ref)) + '</span></span><button class="btn ghost sm" data-cancelfu="' + k + '">Cancel</button></div><div class="msg small">' + esc(w.body || '') + '</div></div>'; }).join('') + '</div>';
    }).join('') + (groups.some(function (g) { return g.wait.length; }) ? '<p class="hint">The later ones book themselves nearer the day.</p>' : '') + '</div>';
    if (!rows.length) {
    ...
    // his own quote, as he sent it (the photo or page one of the PDF), back out with the nudge
    $app.querySelectorAll('[data-qpic]').forEach(function (b) { b.addEventListener('click', function () {
      var r = rows[+b.dataset.qpic], pic = quotePic(r.job); if (!pic) return;
      var who = r.job.client || {}, done = function (res) { if (res && res.sent) { r.job.last_chased = today; save(); viewChase(); } };
      fetch(pic).then(function (x) { return x.blob(); }).then(function (blob) {
        if (ho) ho({ kind: 'quote', file: blob, filename: 'Quote' + (r.job.no_number ? '' : ' ' + r.job.quote_no) + '.jpg', text: r.text.sms, whoName: who.name, whoPhone: who.phone, whoEmail: who.email, job: r.job, onResult: done });
        else downloadBlob(blob, 'Quote.jpg');
      }).catch(function () { toast('That picture would not open.'); });
    }); });
  ...
  function viewSettings() {
    ...
    html += '<div class="card"><h2>Business</h2><div class="g2"><label class="f">Trading name<input type="text" data-bind="details.trading_name"></label><label class="f">Your name<input type="text" data-bind="details.owner_name"></label><label class="f">ABN<span>goes on every quote and invoice</span><input type="text" data-bind="details.abn" inputmode="numeric"></label><label class="f">Mobile<input type="tel" data-bind="details.phone"></label><label class="f">Email<input type="email" data-bind="details.email"></label><label class="f">State<span>sets the deposit limit</span><select data-bind="details.state" id="st_state"><option value="">Pick one</option>' + STATES.map(function (st) { return '<option value="' + st + '">' + st + '</option>'; }).join('') + '</select></label><label class="f">Postcode<span>where you start the day</span><input type="text" data-bind="details.postcode" id="st_pc" inputmode="numeric" placeholder="e.g. 5000"></label>' + (lic ? licField : '') + '</div>' +
      ...
      '<div class="row"><label class="btn ghost sm"><input type="checkbox" data-bind="details.first_name_signoff"> Texts end with ' + (fn && fn !== full ? '"' + esc(fn) + '", not "' + esc(full) + '"' : 'your first name only') + '</label><label class="btn ghost sm ponly"><input type="checkbox" data-bind="details.show_rates"> Show the per-metre prices on the quote</label></div>' +
  ...
  function viewAdd(tab, qs) {
    ...
    if (tab === 'paste') {
      ...
      html += '<div class="card">' + (clip ? '<button class="btn tape lg tile" id="pasteclip"' + QCPics.says('Paste') + '>' + QCPics.tile('paste', 'Paste') + '</button>' : '') + '<label class="f">The text or email you sent<textarea id="pastebox" rows="7" placeholder="Hi Jane, quote for the deck is $2,450 inc GST...">' + esc(viewAdd.pasteText || '') + '</textarea></label><button class="btn tape lg" id="pastego">Read it</button><p class="hint" id="pastemsg"></p></div>';
      $app.innerHTML = html;
  ...
  function tokenCus(t) { try { var b = String(t || '').split('.')[1]; return b ? String(JSON.parse(atob(b.replace(/-/g, '+').replace(/_/g, '/'))).cus || '') : ''; } catch (e) { return ''; } }
  function sendingLinkOk(sd) {
    var mine = String((S.sending || {}).token || ''); if (!mine) return true;   // a new phone: the welcome link sets it up
    var srv = String(sd.server || '').trim(), ours = !srv || /^https:\/\/(www\.)?chasem\.app\//.test(srv) || (location.origin && srv.indexOf(location.origin + '/') === 0);
    if (!ours) return false;
    return !sd.token || String(sd.token) === mine || (!!tokenCus(mine) && tokenCus(sd.token) === tokenCus(mine));   // same account only
  }
  ...
  function applySetup(obj) {
    ...
    // A link can point this phone's sending somewhere only when that is us, or when the phone has no sending of its own
    // yet. Otherwise a link that looks like ours could send his token, and his customers' messages, to someone else.
    if (isObj(st.sending) && !sendingLinkOk(st.sending)) { st = Object.assign({}, st); delete st.sending; }
  ...
  function viewHelp() {
    $app.innerHTML = '<h1>How it works</h1><div class="card"><ol class="steps">' +
      '<li><span><b>Chase.</b> Bring in the quotes you have sent: find them, paste one, a photo or PDF, a file, or type it.</span></li>' +
      '<li class="ponly"><span><b>New job.</b> The client, then each room. A4 on the wall, one photo, confirm the corners.</span></li>' +
      '<li><span><b>Follow up.</b> Days 3, 7 and 14, on weekdays. Every message is on Follow-ups before it goes.</span></li>' +
      '<li><span><b>Won.</b> Tap Won when they say yes, or they tap yes themselves. Book the days, send the deposit.</span></li>' +
      '<li><span><b>Get paid.</b> Reminders after the due date until you mark it paid.</span></li></ol></div>' +
      '<div class="card"><h3>New phone</h3><p class="muted">Open the app and sign in with the same email. Your jobs come back.</p></div>' +
      '<div class="card"><h3>Lost phone</h3><p class="muted">The app lock keeps them out. Sign in on the new one.</p></div>' +
  ...
  (function () { try {
    ...
    // never over a quote he was part-way through typing: then it waits in the Paste box instead
    var had = null; try { had = JSON.parse(localStorage.getItem('qc-add-draft') || 'null'); } catch (e) {}
    var typing = had && !had.found && (String(had.name || '').trim() || String(had.amount || '').trim());
    if (shared.trim() && !typing) draftFrom(shared); else if (shared.trim()) viewAdd.pasteText = shared;
    history.replaceState(null, '', location.pathname + '#/add/' + (shared.trim() && !typing ? 'type' : 'paste'));
  } catch (e) {} })();
  route();
