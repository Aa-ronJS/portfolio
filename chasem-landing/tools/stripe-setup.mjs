#!/usr/bin/env node
// Creates everything Chasem needs inside YOUR Stripe account, in one go, and prints the config to paste.
//
...
(async () => {
  ...
  if (!product && !DRY) product = await stripe("products", { name: "Chasem", description: "Quoting, invoicing and follow-ups for Australian painters.", tax_code: TAX_CODE, "metadata[qc_product]": "main" });
  ...
  if (!hook) {
    hook = await stripe("webhook_endpoints", {
      ...
      description: "Chasem: sign-ups and top-ups",
    });
    secret = hook.secret || "";
    log("  webhook: created");
  ...
  // ---- what to paste
  log("\n================ paste into chasem-landing/public/config.js ================\n");
  ...
  log("\n================ paste into chasem-app/config.js ================\n");
  log("  window.QC_APP = { maps_key: '', signup_url: '" + RELAY + "/api/signup' };");
  log("\n================ set on Vercel (project chasem-landing) ================\n");
