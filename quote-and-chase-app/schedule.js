...
(function () {
  ...
  function roomsFrom(picks, condition, H) { // picks: [{type, size, n}]
    var out = []; (picks || []).forEach(function (p) { var pr = PRESETS[p.type]; if (!pr) return; var f = Math.sqrt(SIZES[p.size] || 1); var cnt = Math.min(50, Math.max(1, parseInt(p.n, 10) || 1)); for (var i = 0; i < cnt; i++) out.push({ id: p.type + i, name: pr[0] + (cnt > 1 ? ' ' + (i + 1) : ''), type: 'interior', method: 'typed', L: +(pr[1] * f).toFixed(1), W: +(pr[2] * f).toFixed(1), H: H || '', walls: [], ceiling_m2: '', condition: condition || 'good', surfaces: { walls: true, ceiling: true, skirting: true }, doors: pr[3], windows: pr[4], doors_one_side: pr[5], wardrobe_pairs: pr[6], feature_m2: 0, wallpaper_m2: 0, ext: {} }); }); return out;
  }
