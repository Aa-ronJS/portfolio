...
(function () {
  ...
  function normaliseJob(j) {
    ...
    j.sent_how = ['text', 'email', 'other'].indexOf(j.sent_how) >= 0 ? j.sent_how : ''; j.sent_confirmed = !!j.sent_confirmed; j.sent_date = j.sent_date == null ? '' : String(j.sent_date); // A1: how the quote went out, set only by the Did-it-go step
    ...
    j.invoices = (Array.isArray(j.invoices) ? j.invoices : []).filter(function (i) { return i && typeof i === 'object'; }).map(function (i) {
      ...
      i.kind = ['deposit', 'progress', 'final', 'full', 'variations'].indexOf(i.kind) >= 0 ? i.kind : 'final'; i.paid_date = i.paid_date || ''; i.sent_how = ['text', 'email', 'other'].indexOf(i.sent_how) >= 0 ? i.sent_how : ''; i.sent_confirmed = !!i.sent_confirmed;
