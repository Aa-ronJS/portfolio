...
(function () {
  ...
  function route() {
    ...
    if (p[0] === 'enquiry') return viewEnquiry(p[1] ? QCStore.getJob(p[1]) : null);
    if (p[0] === 'job' && p[1]) {
  ...
  function viewHome() {
    ...
    var html = '<div class="row between"><h1>Jobs</h1><div class="row"><button class="btn ghost sm" id="newjob">New job</button><a class="btn sm" href="#/enquiry">Phone enquiry</a><button class="btn tape" id="quick">Quick quote</button></div></div><p class="hint">On the phone: Enquiry gives a ballpark and books the visit. On site: Quick quote photographs the walls and sends.</p>';
  ...
  function viewEnquiry(job) {
    ...
    html += '<div class="card"><div class="g2"><label class="f">Name<input type="text" data-bind="client.name" autocomplete="off"></label><label class="f">Mobile<input type="tel" data-bind="client.phone"></label></div><label class="f">Address<span>with postcode, for travel and the slot</span><input type="text" data-bind="client.address" data-refresh="1"></label><label class="f">What they want<input type="text" data-bind="summary" placeholder="Lounge, two bedrooms and the hall, before Christmas"></label></div>';
    html += '<div class="card"><h3>Rooms</h3><p class="hint">Tap a room to add it. Tap again for another. S / M / L is the size.</p><div class="row" id="roomchips">' + Object.keys(QCSched.PRESETS).map(function (k) { return '<button class="btn ghost sm" data-room="' + k + '">' + esc(QCSched.PRESETS[k][0]) + '</button>'; }).join('') + '</div><div id="picks"></div>' +
      '<label class="f">Condition<span>good: clean. fair: scuffs and marks. poor: peeling, damage.</span><select data-bind="condition" data-refresh="1"><option value="good">Good</option><option value="fair">Fair</option><option value="poor">Poor</option></select></label></div>';
    html += '<div class="card"><span class="hint">Ballpark' + (S.details.gst ? ' inc GST' : '') + ', from your price list</span><h2 id="ballpark">' + (job.picks.length ? '' : 'Add rooms') + '</h2><p class="hint" id="bpnote"></p><div class="row"><button class="btn sm" id="textbp">Text them the ballpark</button><button class="btn ghost sm" id="savebp">Save as a job</button></div></div>';
    html += '<div class="card"><h3>Book the quote visit</h3>' + (job.visit ? '<p><b>' + QCPdf.fmtDate(job.visit.date) + ' at ' + QCSched.nice(job.visit.start_min) + '</b> for ' + job.visit.minutes + ' min. ' + esc(job.visit.why || '') + '</p><div class="row"><button class="btn sm" id="visitics">Add to calendar</button><a class="btn ghost sm" href="' + esc(job.visit.gcal || '#') + '" target="_blank" rel="noopener">Google Calendar</a><button class="btn ghost sm" id="textvisit">Text the confirmation</button><button class="btn ghost sm" id="unvisit">Change</button></div>' : '<p class="hint">Suggested times are the ones that add the least driving between what is already booked.</p><div class="row"><label class="f">Visit length, min<input type="number" id="vmin" value="' + esc(S.booking.visit_minutes || 30) + '" min="10" step="5" style="width:6em"></label><button class="btn tape sm" id="findslots">Find a time</button></div><div id="slots"></div>') + '</div>';
    $app.innerHTML = html; bindAll($app, job);
    ...
    refreshPreview = function () {
      ...
      bp.textContent = money(r.low) + ' to ' + money(r.high); var tr = r.priced.travel; note.textContent = r.rooms.length + ' room' + (r.rooms.length > 1 ? 's' : '') + ' at typical sizes, ' + job.condition + ' condition' + (tr && tr.amount ? ', ' + money(tr.amount) + ' travel included' : '') + '. Measured on the day it firms up.';
    };
    ...
    function bpText() { var f = first(job.client.name), det = S.details; return 'Hi ' + f + ', thanks for the call. Ballpark for ' + (job.summary || 'the painting') + ': ' + money(job.ballpark.low) + ' to ' + money(job.ballpark.high) + (det.gst ? ' inc GST' : '') + ', firmed up when I measure on the day. ' + (job.visit ? 'See you ' + QCPdf.fmtDate(job.visit.date) + ' at ' + QCSched.nice(job.visit.start_min) + '. ' : '') + det.sign_off + (det.owner_name ? ', ' + det.owner_name : '') + (det.trading_name ? ', ' + det.trading_name : ''); }
    document.getElementById('textbp').addEventListener('click', function () { if (!job.ballpark) { toast('Add rooms first'); return; } ensureSaved(); var t = bpText(); if (QCMsg.ready('sms') && job.client.phone) { QCMsg.call({ action: 'send', channel: 'sms', to: job.client.phone, body: t }).then(function () { toast('Texted'); }).catch(function (e) { toast('Failed: ' + e.message); }); } else location.href = 'sms:' + (job.client.phone || '').replace(/\s+/g, '') + '?&body=' + encodeURIComponent(t); });
    document.getElementById('savebp').addEventListener('click', function () { ensureSaved(); job.rooms = QCSched.roomsFrom(job.picks, job.condition, S.rules.ceiling_height_m); job.status = 'draft'; save(); toast('Saved as a job with the rooms typed in'); go('/job/' + job.id); });
    var fs2 = document.getElementById('findslots'); if (fs2) fs2.addEventListener('click', function () {
      ...
      if (!res.slots.length) { box.innerHTML = '<p class="confirm">No free slot in the next two weeks. Check your hours in Set-up, Bookings.</p>'; return; }
      box.innerHTML = (res.known ? '' : '<p class="confirm">No postcode I recognise in the address, so these ignore travel.</p>') + res.slots.map(function (sl, i) { return '<button class="btn ' + (i === 0 ? 'tape' : 'ghost') + ' sm" data-slot="' + i + '" style="margin:3px 0"><b>' + QCPdf.fmtDate(sl.date).replace(/ \d{4}$/, '') + ' ' + QCSched.nice(sl.start_min) + '</b>&nbsp;<span class="hint">' + esc(sl.why) + (sl.detour ? ', +' + sl.detour + ' min driving' : '') + '</span></button>'; }).join('');
    ...
    var tv = document.getElementById('textvisit'); if (tv) tv.addEventListener('click', function () { var f = first(job.client.name), det = S.details, t = 'Hi ' + f + ', confirming I will come out to quote on ' + QCPdf.fmtDate(job.visit.date) + ' at ' + QCSched.nice(job.visit.start_min) + '. Takes about ' + job.visit.minutes + ' minutes. ' + det.sign_off + (det.owner_name ? ', ' + det.owner_name : ''); if (QCMsg.ready('sms') && job.client.phone) { QCMsg.call({ action: 'send', channel: 'sms', to: job.client.phone, body: t }).then(function () { toast('Texted'); }).catch(function (e) { toast('Failed: ' + e.message); }); } else location.href = 'sms:' + (job.client.phone || '').replace(/\s+/g, '') + '?&body=' + encodeURIComponent(t); });
  ...
  function viewSettings() {
    ...
    html += '<div class="card"><h2>Bookings and quote visits</h2><div class="g3"><label class="f">Job start<span>hour, 24h</span><input type="number" data-bind="booking.start_hour" min="5" max="12"></label><label class="f">Job finish<input type="number" data-bind="booking.end_hour" min="10" max="20"></label><label class="f">Quote visit, min<input type="number" data-bind="booking.visit_minutes" min="10" step="5"></label><label class="f">Earliest visit<span>hour</span><input type="number" data-bind="booking.quote_from" min="5" max="12"></label><label class="f">Latest visit start<span>hour</span><input type="number" data-bind="booking.quote_to" min="12" max="21"></label></div><div class="row"><label class="btn ghost sm"><input type="checkbox" data-bind="booking.saturdays"> Saturdays</label><label class="btn ghost sm"><input type="checkbox" data-bind="booking.sundays"> Sundays</label></div><p class="hint">Suggested visit times add the least driving between the jobs and visits already booked, using postcode distances from ' + esc(S.details.postcode || 'your postcode') + '.</p></div>';
  ...
  function viewHelp() {
    $app.innerHTML = '<h1>How it works</h1><div class="card"><ol class="steps">' +
      ...
      '<li><span><b>Booked:</b> client says yes, tap Book it in, pick the day, and it lands in your calendar with the address and phone number.</span></li>' +
      '<li><span><b>On the phone:</b> Phone enquiry. Tap the rooms, pick a size, and a ballpark from your own prices appears while they are still talking. Text it to them. Then Find a time: it offers visit slots that add the least driving around what you already have booked, and puts the visit in your calendar.</span></li></ol></div>' +
