...
(async () => {
  ...
  ok(/Quote & Chase/.test(t) && /Your first three jobs are on us, quoted and chased to the end: 12 messages/.test(t) && /\$99 a month for 150/.test(t) && /a pack of 100 more is \$35/.test(t), 'the door states the free jobs, the plan and the pack: ' + t.replace(/\s+/g, ' ').slice(0, 90));
