// Unit test for RELAY_TOKENS (hosted sending) in quote-and-chase-landing/api/msg.js. Run: node relay-tokens.cjs
...
Object.assign(process.env, { TWILIO_ACCOUNT_SID: 'ACserver', TWILIO_AUTH_TOKEN: 'servertok', TWILIO_MESSAGING_SERVICE_SID: 'MGserver', RESEND_API_KEY: 're_server', RESEND_FROM: 'Quote & Chase <hello@quoteandchase.com.au>' });
delete globalThis.__relayFetch;
const handler = require(require('path').join(__dirname, '../../../..', 'quote-and-chase-landing') + '/api/msg.js').default;
...
(async () => {
  ...
  ok(last().json.from === 'Quote & Chase <hello@quoteandchase.com.au>', 'legacy From is RESEND_FROM verbatim (' + last().json.from + ')');
  ...
  ok(r.status === 200 && last().opts.headers.Authorization === 'Bearer re_phone' && last().json.from === 'Quote & Chase <hello@quoteandchase.com.au>', 'legacy ALLOW_CLIENT_CREDS: phone key fills the gap, env From still wins (as today)');
  ...
  process.env.RESEND_FROM = 'Quote & Chase <hello@quoteandchase.com.au>';
