...
Object.assign(process.env, { TWILIO_ACCOUNT_SID: 'ACserver', TWILIO_AUTH_TOKEN: 'twtok', TWILIO_MESSAGING_SERVICE_SID: 'MGserver', RESEND_API_KEY: 're_server', RESEND_FROM: 'Quote & Chase <hello@quoteandchase.com.au>', STRIPE_WEBHOOK_SECRET: 'whsec_test', STRIPE_SECRET_KEY: 'rk_test_x', APP_URL: 'https://chasem.app/app/', BOOKING_URL: 'https://cal.example/aaron/hour', OWNER_MOBILE: '0400 111 222', OWNER_EMAIL: 'aaron@example.com', INBOUND_FORWARD_TO: '0400 111 222' });
const wh = require(require('path').join(__dirname, '../../../..', 'quote-and-chase-landing') + '/api/stripe-webhook.js'), sl = require(require('path').join(__dirname, '../../../..', 'quote-and-chase-landing') + '/api/setup-link.js'), si = require(require('path').join(__dirname, '../../../..', 'quote-and-chase-landing') + '/api/sms-in.js'), U = require(require('path').join(__dirname, '../../../..', 'quote-and-chase-landing') + '/api/_setup.js');
...
(async () => {
  ...
  process.env.TWILIO_INBOUND_URL = 'https://quoteandchase.com.au/api/sms-in'; rr = res(); await si.default(req('POST', form(inbound), { 'content-type': 'application/x-www-form-urlencoded', 'x-twilio-signature': tsig('https://quoteandchase.com.au/api/sms-in', inbound) }, '/api/sms-in'), rr); ok(rr.statusCode === 200 && /<Message>/.test(rr.body), 'TWILIO_INBOUND_URL overrides the URL the signature covers');
