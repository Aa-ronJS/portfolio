...
(async () => {
  ...
  await t.go('#/settings'); await p.waitForSelector('#export'); const [dl] = await Promise.all([p.waitForEvent('download', { timeout: 5000 }).catch(() => null), p.click('#export')]); ok(dl && /quote-and-chase-backup-\d{4}-\d\d-\d\d\.json/.test(dl.suggestedFilename()), 'Save back-up downloads a dated JSON');
  ...
  m = await importText('{}'); ok(/Not a Quote/.test(m), '{} import -> rejected: ' + m);
  m = await importText('[]'); ok(/Not a Quote/.test(m), '[] import -> rejected: ' + m);
  m = await importText('null'); ok(/Not a Quote|Cannot/.test(m) && !t.errors.length, 'null import -> rejected: ' + m);
  m = await importText('"hello"'); ok(/Not a Quote|Cannot/.test(m) && !t.errors.length, '"hello" import -> rejected: ' + m);
