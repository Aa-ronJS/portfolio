...
SUITES="apptest/flow apptest/e2e apptest/sendflow apptest/enquiry apptest/invguard apptest/auto apptest/geom
        ...
        apptest/prefill-e2e apptest/setup-link apptest/calendar-app apptest/landing2 apptest/landing-welcome apptest/council6
...
for t in $SUITES; do
  ...
  if [ $code -ne 0 ] || [ -z "$line" ]; then bad=$((bad+1)); echo "FAILED (exit $code) ${line:-no summary line}"; printf '%s\n' "$out" | grep -E '^FAIL|Error|CRASH|HARNESS' | head -5 | sed 's/^/    /';
  else echo "$line"; fi
done
echo
