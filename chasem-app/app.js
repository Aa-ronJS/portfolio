...
(function () {
  ...
  function setupSteps() { var d = S.details, defaults = (QCStore.defaults && QCStore.defaults().prices) || {}, touched = Object.keys(defaults).some(function (k) { return S.prices[k] !== defaults[k]; });
    return [{ label: 'Your name and business name', done: !!(String(d.trading_name || '').trim() || String(d.owner_name || '').trim()) },
      ...
      { label: 'Make the prices yours', hint: 'One question: what you charge for a day. They are filled in with average rates until you do.', href: '#/myprices', done: !!(S.security.setup_done || touched) },
      { label: 'Let it chase for you', hint: 'Quotes and invoices followed up by text and email in your name, without you. Or keep tapping Send yourself, free.', href: '#/settings', done: !!(S.sending.hosted === true || autoReady()) }]; }
  function viewHome() {
    ...
    if (showSetup) html += '<div class="card" id="setupcard"><div class="row between"><h2>Set up</h2><span class="hint">' + steps.filter(function (s) { return s.done; }).length + ' of ' + steps.length + ' done</span></div><ol class="steps">' + steps.map(function (s) { return '<li class="' + (s.done ? 'done' : '') + '"><span>' + (s.href && !s.done ? '<a href="' + s.href + '">' + esc(s.label) + '</a>' : esc(s.label)) + (s.done ? ' <span class="tag">done</span>' : '') + (s.hint ? '<br><span class="hint">' + esc(s.hint) + '</span>' : '') + '</span></li>'; }).join('') + '</ol><p class="hint">Ten minutes, on your own, whenever suits. Everything else can wait, and you can quote before any of it.</p><div class="row between"><button class="btn tape" id="setupgo">' + (steps[0].done ? 'Finish set-up' : 'Set up') + '</button><button class="btn ghost sm" id="setuphide">Hide this</button></div></div>';
    ...
    var bg = document.getElementById('bookgo'); if (bg) bg.addEventListener('click', function () { bg.disabled = true; var jobs = bookJobs(); if (!(QCMsg.ready('sms') || QCMsg.ready('email'))) { bg.disabled = false; toast('Sending is not on yet. Turn it on in Set-up, or connect your own accounts there.'); return; } queueLoadedFollowUps(jobs).then(function (r) { S.ui.book_chase = 'started'; save(); toast(r.scheduled ? r.scheduled + ' reminder' + (r.scheduled > 1 ? 's' : '') + ' queued for ' + r.jobs + ' job' + (r.jobs > 1 ? 's' : '') + ' in your book. The first goes ' + whenText(r.first) + '.' : 'Nothing to queue: every job in the book already has its reminders, or has no mobile or email.'); viewHome(); }); });
  ...
  function whereItsAt(job) {
    ...
    return '<div class="card"><p>' + lines.join(' ') + '</p><div class="row">' + btns.map(function (b) { return '<a class="btn sm ' + b[2] + '" href="' + b[0] + '">' + b[1] + '</a>'; }).join('') + '</div>' + (st === 'quoted' && job.sent_date ? '<p class="hint">Changes below do not alter the quote ' + who + ' has until you send it again as a revision.</p>' : locked ? '<p class="hint">The quote is locked. Revise quote on the quote page reopens it.</p>' : '') + '</div>';
  }
  ...
  function viewJob(job) {
    ...
    html += '<div class="card"><h2>Client</h2><div class="g2"><label class="f">Client type<select data-bind="client.type" id="ctype"><option value="homeowner">Homeowner</option><option value="agent">Agent</option><option value="strata">Strata</option><option value="builder">Builder</option><option value="commercial">Commercial</option></select></label><label class="f">Name<span>person or business</span><input type="text" data-bind="client.name" autocomplete="off"></label><label class="f">Contact first name<span>for texts</span><input type="text" data-bind="client.first_name" autocomplete="off"></label><label class="f">Mobile<input type="tel" data-bind="client.phone"></label></div>' + (typeLine ? '<p class="hint" id="ctypeline">' + esc(typeLine) + '</p>' : '') +
      '<label class="f">Email<input type="email" data-bind="client.email"></label><div id="bizf" class="g2" ' + (biz ? '' : 'hidden') + '><label class="f">Accounts email<span>where invoices and statements go, if not the contact</span><input type="email" data-bind="client.accounts_email"></label><label class="f">Client ABN<span>only if they ask for it on the invoice</span><input type="text" data-bind="client.abn" inputmode="numeric"></label></div>' +
  ...
  function presetLine(job, rid, room, rerender) { var chips = document.getElementById('presets'); if (!chips) return; var el = document.getElementById('presetline'); if (!el) { el = document.createElement('p'); el.id = 'presetline'; el.className = 'hint'; chips.parentNode.insertBefore(el, chips.nextSibling); } if (!presetPending || presetPending.rid !== rid) { el.textContent = ''; el.hidden = true; return; }
    ...
    el.hidden = false; el.className = 'hint presetline'; el.innerHTML = '<span><b>Filled in:</b> ' + esc(items.join(', ')) + '.</span> <button type="button" class="btn ghost sm" id="presetkeep">Keep</button><button type="button" class="btn ghost sm" id="presetclear">Clear</button><span class="hint">Clear takes off the doors, windows and skirting; the sizes stay.</span>';
  ...
  function viewRoom(job, rid) {
    ...
    if (ext) {
      ...
      html += '<div class="g3"><label class="f">Condition<span>fair = chalky, poor = peeling</span><select data-bind="ext.condition" data-refresh="1" id="extcond"><option value="good">Good</option><option value="fair">Fair</option><option value="poor">Poor</option></select></label><label class="f">Storeys<select data-bind="ext.storeys" data-refresh="1" id="extstoreys"><option value="1">Single storey</option><option value="2">Two storey</option></select></label><label class="f">Coats<select data-bind="ext.coats" data-refresh="1"><option value="2">2</option><option value="3">3</option></select></label></div>' +
        '<div id="scopebox" ' + (scopeOn ? '' : 'hidden') + '><label class="f">How much of it is like that<span>prep is priced on the worst side; say how much of the house is like it</span><select data-bind="ext.condition_scope" data-refresh="1"><option value="all">All of it</option><option value="half">About half</option><option value="side">One side</option></select></label></div>' +
    ...
    if (!ext) {
      ...
      ensureMount();
      // LiDAR import
      var imp = document.createElement('div'); imp.className = 'row'; imp.innerHTML = '<details class="adv" style="width:100%"><summary>Other ways to measure</summary><p class="hint">Got a room scan from an iPhone Pro (RoomPlan)? Import it and the walls come in measured. Otherwise type the sizes and say who measured them.</p><label class="btn ghost sm">Import a room scan (RoomPlan JSON)<input type="file" accept=".json,application/json" id="lidar"></label></details>';
  ...
  function didItGo(o) {
    ...
    box.innerHTML = (o.handed === 'downloaded' ? '<p>Your phone saved the ' + esc(word) + ' as a file. Open it from your Files app to send it, or tap Send and choose Messages or Mail.</p>' : o.handed === 'copied' ? '<p>The words are copied. Paste them into a text or an email.</p>' : o.handed === 'cancelled' ? '<p>The menu closed without sending, as far as this app can tell.</p>' : '') +
  ...
  function viewQuote(job, keep) {
    ...
    html += '<p class="hint">' + (view.measured_rooms ? view.measured_rooms + ' of ' + view.total_rooms + ' rooms measured from photos. ' : (view.total_rooms ? 'Fixed price from the sizes you typed. ' : '')) + depTxt + '</p>';
    ...
    var sendNote = locked ? 'Send again sends the quote exactly as accepted. ' : autoSend ? 'Emails the quote to ' + esc(job.client.email) + ' from here. ' : 'You see the quote first. Then tap Send and pick <b>Messages</b> (to text it to ' + esc(whoShort) + ') or <b>Mail</b>. Nothing goes until you press Send in that app. ';
    ...
    var foot = sent ? 'Sent to ' + esc(who) + ' ' + howWord(job.sent_how) + ', ' + fmtShort(qSent) + '.' : (job.handed_at ? 'Handed to your phone ' + fmtShort(job.handed_at) + '. Not sent yet.' : 'Not sent yet.');
    ...
    if (q) foot += ' Quote ' + esc(q.number || job.quote_no) + ' dated ' + QCPdf.fmtDate(q.date) + (q.snapshot ? ', wording and terms frozen with it' : '') + '.';
  ...
  function viewInvoice(job, keep, qs) {
    ...
    if (job.invoices.length) {
      html += '<div class="card"><h3>Invoices</h3>' + job.invoices.map(function (i, k) {
        ...
        s += '<div id="voidform_' + k + '" class="scalebox" hidden><b>Cancel invoice ' + esc(i.no) + '</b><span class="hint">The number stays used and its copy is stamped VOID. It counts for nothing after this. Nothing is sent to ' + esc(whoShort) + '.</span><label class="f">Why<input type="text" id="void_why_' + k + '" placeholder="Wrong amount, reissued as INV-..."></label><div class="row"><button class="btn danger sm" data-voidgo="' + k + '">Yes, cancel this invoice</button></div></div>';
        s += '<div id="cnform_' + k + '" class="scalebox" hidden><b>Refund or credit on ' + esc(i.no) + '</b><span class="hint">Takes an amount off this paid invoice. It becomes a credit on the job: refund it, or take it off the next invoice.</span><div class="g2"><label class="f">Amount inc GST<input type="number" step="0.01" min="0.01" id="cn_amt_' + k + '" value="' + i.total + '"></label><label class="f">Why<input type="text" id="cn_why_' + k + '" placeholder="Work not done, agreed discount"></label></div><div class="row"><button class="btn tape sm" data-cngo="' + k + '">Issue the credit</button></div></div>';
        return s + '</div>';
    ...
    if (!kinds.length) html += '<div class="card"><p class="muted">' + (cancelled ? 'No new invoices on a cancelled job.' : 'Final invoice ' + esc(finals[finals.length - 1].no) + ' issued. Extra work goes on the job as a variation, then here.') + '</p></div>';
      ...
      '<div id="invprev"></div><p id="billline"></p><div class="row" id="mkrow"><button class="btn tape" id="mkinv">Send invoice</button>' + (S.stripe && S.stripe.enabled ? (QCStripe.keyLooksRight(S.stripe.key) ? '<span class="hint">A card payment link goes on it.</span>' : '<span class="confirm">Card payments key not valid. Fix in Set-up.</span>') : '') + '</div><p class="hint">You see the invoice first. Then tap Send and pick Messages or Mail. Nothing goes until you press Send in that app.</p></div>';
  ...
  function viewEnquiry(job) {
    ...
    html += '<div class="card"><h3>Rooms</h3><label class="f">What<span>a walls-only repaint is about a third of the full job</span></label><div class="chips" id="scopechips">' + SCOPE_OPTS.map(function (o) { return '<button class="chip' + (job.scope === o[0] ? ' on' : '') + '" data-scope="' + o[0] + '">' + o[1] + '</button>'; }).join('') + '</div><p class="hint">Tap to add rooms. S / M / L = size.</p><div class="row" id="roomchips">' + Object.keys(QCSched.PRESETS).map(function (k) { return '<button class="btn ghost sm" data-room="' + k + '">' + esc(QCSched.PRESETS[k][0]) + '</button>'; }).join('') + '</div><div id="picks"></div>' +
    ...
    refreshPreview = function () {
      var bp = document.getElementById('ballpark'), note = document.getElementById('bpnote'); if (!job.picks.length) { bp.textContent = 'Add rooms'; note.textContent = 'Then you get a rough range for ' + SCOPE_SAY[job.scope] + '.'; job.ballpark = null; return; }
    ...
    var fs2 = document.getElementById('findslots'); if (fs2) fs2.addEventListener('click', function () {
      ...
      box.innerHTML = (res.known ? '' : '<p class="confirm">No postcode in the address, so driving time is not counted.</p>') + (clear ? '<p class="hint">Nothing else booked yet, so any of these work. Pick one:</p>' : '') +
  ...
  function testBar() {
    ...
    el.hidden = false;
    el.innerHTML = '<span><b>Test drive.</b> ' + (t.divert === false
      ? 'Messages are going to the real numbers on the jobs.'
      : (mine ? 'Every text and email comes to ' + esc(mine) + '.' : '<b>No mobile or email of yours yet</b>, so nothing can be sent.')) + '</span>' +
      '<span><a href="#/test">Walkthrough</a> &nbsp; <a href="#" id="testoff">Turn off</a></span>';
    var off = document.getElementById('testoff');
    if (off) off.addEventListener('click', function (e) { e.preventDefault(); QCTest.set({ on: false }); toast('Test drive off. Sending goes to your customers again.'); route(); });
  }
  ...
  function viewTest() {
    ...
    html += '<div class="card"><p>' + (t.on
      ? 'Nothing you do in here can reach a customer: every text and email is re-addressed to you, with a line at the top saying where it would have gone. Everything else is real \u2014 the same relay, the same Twilio, the same replies coming back.'
      : 'Turn this on before you test anything that sends. While it is on, every text and email comes to you instead of to the name on the job.') + '</p>' +
      (t.on && !mineOk ? '<p class="confirm">Put your own mobile and email in Set-up first, or there is nowhere for the test messages to go.</p>' : '') +
      '<div class="row"><button class="btn ' + (t.on ? 'ghost' : 'tape') + '" id="tdtoggle">' + (t.on ? 'Turn test drive off' : 'Turn test drive on') + '</button>' +
      (t.on ? '<label class="btn ghost sm"><input type="checkbox" id="tddivert"' + (t.divert === false ? '' : ' checked') + '> Send everything to me</label>' : '') + '</div>' +
      (t.on && t.divert === false ? '<p class="confirm">Messages are going to the real numbers and addresses on the jobs. Only leave this off if every job in your book is one of your own numbers.</p>' : '') + '</div>';

    html += '<div class="card"><h2>Your book</h2><p class="hint">Eight jobs at every stage, dated so the chasing is live the moment it lands: a quote waiting 12 days, a deposit unpaid, an invoice 21 days overdue, a job booked for next week.</p>' +
      '<div class="row"><button class="btn tape" id="tdseed">Load a busy week</button><button class="btn ghost" id="tdempty">Start empty, like a new painter</button></div>' +
      '<p class="hint">Both wipe what is on this phone first. If you have anything real in here, take a back-up from Set-up before you tap either.</p></div>';

    html += '<div class="row between"><h2>Walk it through</h2><span class="hint">' + done + ' of ' + steps.length + ' looked at' + (broken ? ' \u00b7 ' + broken + ' broken' : '') + '</span></div>';
    steps.forEach(function (st, i) {
      var f = byStep[st.id], cls = f ? (f.verdict === 'broken' ? ' bad' : f.verdict === 'works' ? ' done' : '') : '';
      // a step he has already ruled on folds away, so the list gets shorter as he works down it
      html += '<details class="step' + cls + '" data-step="' + st.id + '"' + (f ? '' : ' open') + '>' +
        '<summary><h3><span>' + (f ? (f.verdict === 'broken' ? '\u2715 ' : '\u2713 ') : '') + esc(st.t) + '</span><span class="n">' + (i + 1) + ' of ' + steps.length + '</span></h3></summary>' +
        '<p>' + esc(st.d) + '</p>' +
        '<div class="row"><a class="btn sm" href="' + esc(st.go) + '">Open it</a>' +
        '<button class="btn ghost sm" data-td="works" data-for="' + st.id + '">It works</button>' +
        '<button class="btn danger sm" data-td="broken" data-for="' + st.id + '">Something is wrong</button></div>' +
        '<textarea data-note="' + st.id + '" rows="2" placeholder="What happened? What did you expect instead?">' + esc(f ? f.note : '') + '</textarea>' +
        (f ? '<p class="verdict ' + (f.verdict === 'broken' ? 'bad' : 'ok') + '">' + (f.verdict === 'broken' ? 'Marked as broken' : 'Marked as working') + (f.sent ? ', sent' : ', on this phone') + '.</p>' : '') +
        '</details>';
    });

    html += '<div class="card"><h2>Anything else</h2><p class="hint">Something that is not on the list: an idea, a word that reads wrong, a screen that feels slow.</p>' +
      '<textarea id="tdloose" rows="3" placeholder="Anything at all."></textarea><div class="row"><button class="btn sm" id="tdadd">Add it</button></div></div>';

    if (found.length) {
      html += '<div class="card"><h2>What you have found</h2><ul class="found">' +
        found.slice().reverse().map(function (f) {
          return '<li>' + (f.verdict === 'broken' ? '<span class="b">Broken</span> ' : f.verdict === 'works' ? '' : '') +
            '<b>' + esc(f.step || 'Note') + '</b>' + (f.note ? ' \u2014 ' + esc(f.note) : '') + (f.sent ? ' <span class="hint">(sent)</span>' : '') + '</li>';
        }).join('') + '</ul>' +
        '<div class="row"><button class="btn tape" id="tdsend">Send them in</button><button class="btn ghost sm" id="tdcopy">Copy them</button><button class="btn ghost sm" id="tdclear">Clear the list</button></div>' +
  ...
  function wireTest() {
    ...
    if (tog) tog.addEventListener('click', function () { QCTest.set({ on: !QCTest.state().on }); toast(QCTest.state().on ? 'Test drive on. Nothing can reach a customer.' : 'Test drive off.'); route(); });
    ...
    if (seed) seed.addEventListener('click', function () {
      if (!confirm('This wipes what is on this phone and loads eight test jobs. Carry on?')) return;
      ...
      toast('Eight jobs loaded. Test drive is on.'); go('/');
    });
    var empty = document.getElementById('tdempty');
    if (empty) empty.addEventListener('click', function () {
      if (!confirm('This wipes what is on this phone and leaves it empty. Carry on?')) return;
      var mine = { phone: S.details.phone, email: S.details.email };
      QCTest.wipe(); S = QCStore.load(); QCTest.seedSettings(mine); QCTest.set({ on: true }); S = QCStore.load();
      toast('Empty, and set up as a new painter would be.'); go('/');
    });
    ...
    Array.prototype.forEach.call($app.querySelectorAll('[data-td]'), function (b) {
      b.addEventListener('click', function () {
        ...
        if (b.getAttribute('data-td') === 'broken' && !String(note).trim()) { showBlock(b, 'Say what went wrong first, even in four words. A tick with nothing behind it cannot be fixed.', { kind: 'warn' }); if (ta) ta.focus(); return; }
        QCTest.record(id, b.getAttribute('data-td'), note);
        toast(b.getAttribute('data-td') === 'broken' ? 'Written down.' : 'Marked as working.');
        viewTest();
      });
    });
    ...
    if (copy) copy.addEventListener('click', function () {
      ...
      try { navigator.clipboard.writeText(txt); toast('Copied. Paste it anywhere.'); }
      catch (e) { var out = document.getElementById('tdres'); out.textContent = txt; }
    });
    var clr = document.getElementById('tdclear');
    if (clr) clr.addEventListener('click', function () { if (confirm('Clear everything you have written down?')) { QCTest.clearFound(); viewTest(); } });
  }
  ...
  function viewSettings() {
    ...
    html += '<div class="card"><h2>Deposit and payment terms</h2><div class="g3"><label class="f">Deposit %<span>homeowners; change per job</span><input type="number" data-bind="details.deposit_pct" id="st_dep" min="0" max="100" step="0.5"></label><label class="f">Deposit due, days<span>after they accept</span><input type="number" data-bind="rules.deposit_due_days" min="0" placeholder="5"></label><label class="f">Payment days<span>after the invoice</span><input type="number" data-bind="details.balance_days" id="st_days" min="0"></label><label class="f">Quote valid, days<input type="number" data-bind="details.quote_valid_days" min="1"></label></div><p class="hint" id="depcap">' + esc(depCapNote()) + '</p>' +
      '<h3>Business clients</h3><p class="hint">Agents, strata, builders and commercial jobs. Used when you pick that client type on a job; change it on the job if one is different.</p><div class="g2"><label class="f">Deposit %<span>0 = no deposit</span><input type="number" data-bind="rules.business_deposit_pct" min="0" max="100" step="0.5" placeholder="0"></label><label class="f">Payment days<input type="number" data-bind="rules.business_days" min="0" placeholder="30"></label></div></div>';
    ...
    html += '<div class="card"><h2>Prices</h2><p class="hint">These five make most quotes. They are filled in; change any you like. Before GST, labour and paint included.</p><table><thead><tr><th>Item</th><th class="n">$</th></tr></thead><tbody>' + five.map(function (p) { return priceRow(p, FIVE[p[0]]); }).join('') + '</tbody></table>' +
    ...
    (function () {
      var t = window.QCTest && QCTest.state();
      html += '<div class="card"><h2>Test drive</h2><p class="hint">' + (t && t.on
        ? '<b>On.</b> Nothing you do can reach a customer: every text and email comes to you instead.'
        : 'Walk the whole job through on purpose, on jobs that are not real, with nothing able to reach a customer. Fourteen steps, and somewhere to write down what breaks.') +
        '</p><div class="row"><a class="btn ' + (t && t.on ? 'tape' : 'ghost') + ' sm" href="#/test">Open the test drive</a></div></div>';
    })();
    html += calendarCard();
    ...
    html += '<div class="card"><h2>Advanced: work prices out from my costs</h2><p class="hint">Optional. Type what an hour and a litre cost you and the app can fill in the price list for you. Nothing here is needed to send a quote.</p><div class="g3"><label class="f">An hour costs you $<input type="number" data-bind="costing.labour_rate" min="0"></label><label class="f">Markup on cost %<span id="mkhint">' + esc(markupNote()) + '</span><input type="number" data-bind="costing.margin_pct" id="st_markup" min="0"></label><label class="f">Painters on a job<input type="number" data-bind="costing.crew" min="1" max="20" placeholder="1"></label><label class="f">Hours per day<input type="number" data-bind="costing.hours_per_day" min="1" max="14" step="0.5" placeholder="8"></label><label class="f">Coats<input type="number" data-bind="costing.coats" min="1" max="3"></label><label class="f">Speed<span>1 = typical, 0.8 = faster, 1.2 = slower</span><input type="number" step="0.1" data-bind="costing.hours_scale" min="0.5"></label><label class="f">Spare paint %<span>extra paint built into the rates</span><input type="number" data-bind="costing.wastage_pct" min="0" max="50" placeholder="8"></label><label class="f">Set-up hours per job<input type="number" step="0.25" data-bind="costing.setup_hours" min="0" placeholder="1.5"></label><label class="f">Extra hours per day<span>set-up, clean-up</span><input type="number" step="0.25" data-bind="costing.daily_hours" min="0" placeholder="0.75"></label></div>' +
      ...
      '<div class="row"><button class="btn sm" id="derive">Calculate prices</button><span class="hint">Shows you each change first, then fills in the price list.</span></div></div>';
  ...
  function depCapNote() {
    var st = stateOf(), pct = defaultDepPct(), line = { NSW: 'In NSW the law caps a deposit at 10%.', VIC: 'In Victoria the law caps a deposit at 10% (5% on jobs of $20,000 and over).', QLD: 'In Queensland the law caps a deposit at 10% (5% on jobs of $20,000 and over).', SA: 'In South Australia the law caps a deposit at $1,000 on jobs up to $20,000 (5% above that).', WA: 'In WA the law caps a deposit at 6.5% on jobs of $7,500 and over.' }[st];
    if (!st) return 'Pick your state under Business and this line tells you the deposit limit there.';
  ...
  function viewMyPrices() {
    ...
    var html = '<a class="hint" href="#/settings">&larr; Set-up</a><h1>Make the prices yours</h1>' +
      '<p class="hint">Five minutes, and the app quotes the way you do. The prices in it now are middle-of-the-road Australian rates; they work, but they are not yours. Pick whichever of these you can answer. Nothing leaves the phone.</p>';
    html += '<div class="card"><h3>1. What do you charge for a day on the tools?</h3><p class="hint">Your day rate for one painter, before GST. Everything in the price list is worked out from it, at your markup.</p>' +
      '<div class="g2"><label class="f">A day, one painter<span>e.g. 520</span><input type="number" id="mp_day" min="0" step="10" value="' + (day > 0 ? day : '') + '" inputmode="numeric"></label>' +
      '<label class="f">Markup on your costs %<span>what covers the ute, insurance and your time quoting</span><input type="number" id="mp_margin" min="0" max="300" step="5" value="' + esc(String((S.costing && S.costing.margin_pct) != null ? S.costing.margin_pct : 40)) + '"></label></div>' +
      '<div class="row"><button class="btn tape sm" id="mp_daygo">Work my prices out</button><span class="hint" id="mp_dayres"></span></div></div>';
    html += '<div class="card"><h3>2. Or: a job you have already quoted</h3><p class="hint">Take a room you priced yourself and tell the app what you charged. It moves every price by the same amount, so the whole list lands where you would have put it.</p>' +
    ...
    html += '<div class="card"><h3>3. Or type them in</h3><p class="hint">The five that make most quotes are on the Set-up page, with the rest underneath.</p><div class="row"><a class="btn ghost sm" href="#/settings/prices">Open the price list</a>' + (pricesTouched() ? '<button class="btn ghost sm" id="mp_reset">Back to the starter prices</button>' : '') + '</div></div>';
    $app.innerHTML = html;
  ...
  function viewHelp() {
    $app.innerHTML = '<h1>How it works</h1><div class="card"><ol class="steps">' +
      ...
      '<li><span><b>New job.</b> Type the client, add each room. Type the sizes, or tap a typical room and fix what differs.</span></li>' +
      ...
      '<li><span><b>Quote.</b> Tap Look at the quote to read it, then Send: your phone opens Messages or Mail with the quote attached, and you press send there. If a price is missing, that line says TO CONFIRM; fill it in first.</span></li>' +
      '<li><span><b>Follow up.</b> The app writes three follow-up texts, for 3, 7 and 14 days after the quote. On the day, open Follow-ups and tap Text: Messages opens with the words ready and you press send. Nothing goes without you unless you set up automatic sending (below).</span></li>' +
      ...
      '<li><span><b>Back up.</b> Save a copy after every invoice. It is one file; keep it in your email or Drive.</span></li></ol></div>' +
      '<div class="card"><h3>Where your data lives</h3><p class="muted">On this phone. Nothing is uploaded anywhere. Clearing the browser data wipes the app, so keep a back-up copy.</p></div>' +
      '<div class="card"><h3>Lost your phone, or getting a new one?</h3><p class="muted">Install the app on the new phone, open Set-up, Back-up, and tap Restore from file with your last back-up copy. Your jobs, quotes, invoices and settings come back. If you had a PIN, nobody can open the app on the lost phone without it.</p></div>' +
      '<div class="card"><details class="adv"><summary>If you set up automatic sending</summary>' +
      '<p class="muted">Optional. It needs a computer and about an hour (Set-up, SMS and email). With it, follow-ups and reminders go by themselves on the day; without it, the app writes them and you tap Text.</p>' +
      '<p class="muted"><b>Lost your phone?</b> Your sending keys are not in the back-up: log in to Twilio, Resend and Stripe on a computer, delete the old keys and make new ones, then type them into Set-up. Change the relay token on your relay.</p>' +
      '<p class="muted"><b>Moving to a new phone?</b> On the old phone: Set-up, Back-up, tick "Include keys", Save back-up, and send the file to yourself. On the new phone: install the app, Set-up, Restore from file. Then untick "Include keys". Delete the back-up file from your messages afterwards; it holds your keys.</p>' +
      '<p class="muted"><b>What leaves the phone.</b> Only the texts and emails you send, through your own Twilio, Resend and relay accounts, and card links through your own Stripe.</p></details></div>';
  }
  function viewSentLog() {
    ...
    var html = '<a class="hint" href="#/settings">&larr; Set-up</a><h1>Sent log</h1><p class="muted">Every text and email sent, scheduled or cancelled from this phone, and every quote or invoice handed to your phone to send. Newest first. Your record if a client ever asks. The last 500 are kept and go in the back-up.</p>';
