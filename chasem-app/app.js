...
(function () {
  ...
  function viewHome() {
    ...
    if (onPhone && !standalone && !window.__qcInstallPrompt) html += '<p class="hint">' + (/Android/i.test(navigator.userAgent) ? 'To keep it on your home screen: Chrome menu, then Install app.' : 'To keep it on your home screen: tap Share, then Add to Home Screen.') + ' It works offline.</p>';
  ...
  function whereItsAt(job) {
    ...
    return '<div class="card"><p>' + lines.join(' ') + '</p><div class="row">' + btns.map(function (b) { return '<a class="btn sm ' + b[2] + '" href="' + b[0] + '">' + b[1] + '</a>'; }).join('') + '</div>' + (st === 'quoted' && job.sent_date ? '<p class="hint">Changes here do not reach ' + who + ' until you send a revision.</p>' : locked ? '<p class="hint">Locked. Revise quote reopens it.</p>' : '') + '</div>';
  }
  ...
  function fuHint() { return autoReady() ? 'Follow-up texts and emails go by themselves on the day. Hold reminders stops them.' : 'Written on the day. You tap Send.'; }
  function viewJob(job) {
    ...
    html += '<div class="card"><div class="row between"><h2>Extras</h2><button class="btn ghost sm" id="addextra">+ Line</button></div><p class="hint">Not in a room. Confirm if the price is a guess.</p><div id="extras"></div>' +
    ...
    html += '<div class="card"><div class="row between"><h2>Colours</h2><button class="btn ghost sm" id="addcol">+ Colour</button></div><p class="hint">Printed on the quote.</p><div id="cols"></div></div>';
  ...
  function viewQuote(job, keep) {
    ...
    var sendNote = locked ? 'Send again sends the quote exactly as accepted. ' : autoSend ? 'Emails the quote to ' + esc(job.client.email) + ' from here. ' : 'Read it, then Send: <b>Messages</b> (' + esc(whoShort) + ') or <b>Mail</b>. You press send there. ';
  ...
  function viewInvoice(job, keep, qs) {
    ...
    if (job.invoices.length) {
      html += '<div class="card"><h3>Invoices</h3>' + job.invoices.map(function (i, k) {
        ...
        s += '<div id="voidform_' + k + '" class="scalebox" hidden><b>Cancel invoice ' + esc(i.no) + '</b><span class="hint">The number stays used, the copy is stamped VOID. Nothing is sent to ' + esc(whoShort) + '.</span><label class="f">Why<input type="text" id="void_why_' + k + '" placeholder="Wrong amount, reissued as INV-..."></label><div class="row"><button class="btn danger sm" data-voidgo="' + k + '">Yes, cancel this invoice</button></div></div>';
  ...
  function viewEnquiry(job) {
    ...
    html += '<div class="card"><h3>Rooms</h3><label class="f">What<span>about a third of the full job</span></label><div class="chips" id="scopechips">' + SCOPE_OPTS.map(function (o) { return '<button class="chip' + (job.scope === o[0] ? ' on' : '') + '" data-scope="' + o[0] + '">' + o[1] + '</button>'; }).join('') + '</div><p class="hint">S / M / L.</p><div class="row" id="roomchips">' + Object.keys(QCSched.PRESETS).map(function (k) { return '<button class="btn ghost sm" data-room="' + k + '">' + esc(QCSched.PRESETS[k][0]) + '</button>'; }).join('') + '</div><div id="picks"></div>' +
  ...
  function viewSettings() {
    ...
    html += '<div class="card"><h2>Deposit and payment terms</h2><div class="g3"><label class="f">Deposit %<span>homeowners; change per job</span><input type="number" data-bind="details.deposit_pct" id="st_dep" min="0" max="100" step="0.5"></label><label class="f">Deposit due, days<span>after they accept</span><input type="number" data-bind="rules.deposit_due_days" min="0" placeholder="5"></label><label class="f">Payment days<span>after the invoice</span><input type="number" data-bind="details.balance_days" id="st_days" min="0"></label><label class="f">Quote valid, days<input type="number" data-bind="details.quote_valid_days" min="1"></label></div><p class="hint" id="depcap">' + esc(depCapNote()) + '</p>' +
      '<h3>Business clients</h3><p class="hint">Agents, strata, builders, commercial. Change it on the job if one is different.</p><div class="g2"><label class="f">Deposit %<span>0 = no deposit</span><input type="number" data-bind="rules.business_deposit_pct" min="0" max="100" step="0.5" placeholder="0"></label><label class="f">Payment days<input type="number" data-bind="rules.business_days" min="0" placeholder="30"></label></div></div>';
    ...
    html += '<div class="card"><h2>Automatic texting and emailing</h2><p class="hint">' + (hostedEnded ? '<span class="confirm">The chasing is off: your sending ran to ' + esc(hostedTo) + '. Turn it back on at <a href="' + esc(SITE + '#price') + '" target="_blank" rel="noopener">the website</a> and one tap in the email puts it back, or connect your own accounts below. Nothing on this phone is lost.</span>' : hostedStopped ? '<span class="confirm">Cancelled. The chasing keeps running to ' + esc(hostedTo) + ' and stops after that. Change your mind any time with Manage below.</span>' : hosted ? 'On, and paid' + (hostedTo ? ' to ' + esc(hostedTo) + ', when it renews itself' : '') + '. Texts and emails go out in your name and there is nothing to open. Every text it sends ends with a way to reach you' + (String(S.details.phone || '').trim() ? ' on ' + esc(String(S.details.phone).trim()) + ', because the number it comes from cannot take replies.' : (String(S.details.email || '').trim() ? ' by email, because the number it comes from cannot take replies. <span class="confirm">Add your mobile in Your business above and texts point them there instead.</span>' : ', because the number it comes from cannot take replies. <span class="confirm">Add your mobile in Your business above so there is one to give.</span>')) : 'Your own texting and emailing accounts, instead of ours. Almost nobody needs this: it takes a computer and about an hour, and your plan already does the sending. Without either, the app writes every text and email and you press Send yourself; nothing goes out without you.' + (autoReady() ? ' Set up and working.' : ' Rather not? <a href="' + esc(SITE + '#price') + '" target="_blank" rel="noopener">Turn the chasing on</a>: one card, one tap, cancel from this page any time.')) + '</p>' +
      ...
      (hosted && QCMsg.balance().plan === 'paid' ? (function () { var st2 = seats();
        ...
        if (st2.seats > 1) return '<div class="card sub" id="seatcard"><h3>Your second phone</h3><p class="hint">Same business name, same messages, its own jobs. A new link replaces the second phone, so a lost handset is just this button.</p><div class="row"><button class="btn ghost sm" id="seatgo">Set up the second phone</button><span class="hint" id="seatres"></span></div><div id="seatout" hidden></div></div>';
  ...
  function viewMyPrices() {
    ...
    var html = '<a class="hint" href="#/settings">&larr; Set-up</a><h1>Make the prices yours</h1>' +
      '<p class="hint">Five minutes, and the app quotes the way you do. The prices in here are average Australian rates. Answer whichever suits. Nothing leaves the phone.</p>';
    html += '<div class="card"><h3>1. What do you charge for a day on the tools?</h3><p class="hint">One painter, a day, before GST.</p>' +
    ...
    html += '<div class="card"><h3>2. Or: a job you have already quoted</h3><p class="hint">A room you priced yourself, and what you charged. Every price moves to match.</p>' +
    ...
    html += '<div class="card"><h3>3. Or type them in</h3><p class="hint">The main five are on Set-up.</p><div class="row"><a class="btn ghost sm" href="#/settings/prices">Open the price list</a>' + (pricesTouched() ? '<button class="btn ghost sm" id="mp_reset">Back to the starter prices</button>' : '') + '</div></div>';
    $app.innerHTML = html;
  ...
  function viewSentLog() {
    ...
    var html = '<a class="hint" href="#/settings">&larr; Set-up</a><h1>Sent log</h1><p class="muted">Newest first. The last 500. go in the back-up.</p>';
