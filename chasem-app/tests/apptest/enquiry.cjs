...
(async () => {
  ...
  await p.click('a[href="#/enquiry"]'); await p.waitForSelector('#findslots');
  ...
  await p.click('a[data-nav="home"]'); await p.click('a[href="#/enquiry"]'); await p.waitForSelector('#findslots'); await p.fill('[data-bind="client.address"]', '5 Hill Rd Mount Barker SA 5251'); await p.click('#findslots'); await p.waitForSelector('[data-slot]');
