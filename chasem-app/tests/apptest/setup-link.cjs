...
(async () => {
  ...
  await p.evaluate(start => {
    const st = window.__qcApp.store; st.reset(); const S = st.load(); S.details.trading_name = S.details.trading_name || 'Test Painting Co'; S.details.abn = S.details.abn || '12 345 678 901'; S.details.state = S.details.state || 'SA'; S.security.setup_done = true; S.payment = S.payment || { account_name: 'Test Painting Co', bsb: '063-000', account_number: '12345678' }; const T0 = 0, T = st.today(), D = n => st.addDays(T, n); S.account = { email: 'test@example.com', joined: T, verified: true }; // reset wipes the account too, as it should
