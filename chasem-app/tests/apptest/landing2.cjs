...
(async () => {
  ...
  await p.goto(base + 'prefill', { waitUntil: 'load' }); await p.fill('#trading_name', 'Dave Painting'); await p.selectOption('#state', 'VIC'); await p.fill('#prices', 'p_walls = 24'); await p.fill('#jobs', 'Margaret Hanley | 0411 222 333 | 8 Beaumont St | Lounge | 2450 | quoted'); await p.click('#build'); await p.waitForFunction(() => /#\/setup\?d=/.test(document.getElementById('out').textContent)); const link = await p.$eval('#out', e => e.textContent); ok(/^https:\/\/go\.chasem\.app\/#\/setup\?d=(z|j):/.test(link), 'link builder makes a set-up link on the real domain: ' + link.slice(0, 70));
