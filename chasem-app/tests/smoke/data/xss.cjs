...
(async () => {
  ...
  async function seed(payload) {
    return p.evaluate(pl => {
      ...
      S.account = { email: 'test@example.com', joined: store.today(), offline: true, verified: true }; S.details.trading_name = S.details.trading_name || 'Test Painting Co'; S.details.abn = S.details.abn || '12 345 678 901'; S.details.state = S.details.state || 'SA'; S.security.setup_done = true; S.payment = S.payment || { account_name: 'Test Painting Co', bsb: '063-000', account_number: '12345678' };  // reset wipes the account; the app shows the join screen without one
