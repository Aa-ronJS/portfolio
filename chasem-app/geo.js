...
(function () {
  ...
  function travel(job, settings, days) {
    ...
    var d = distance(from, to, parseFloat(rules.road_factor) || 1.2); if (!d) return { km: 0, chargeable: 0, per_trip: 0, days: nDays, amount: 0, source: 'postcode not recognised', note: 'Postcode ' + (centroid(from) ? to : from) + ' not recognised. No travel charged.' };
    ...
    return { km: d.km, chargeable: ch, per_trip: per, days: nDays, amount: amount, source: 'postcode ' + to + ' from ' + from, note: ch > 0 ? (d.km + ' km. ' + ch + ' km charged' + (both ? ' each way' : '') + ' at $' + rate + '/km' + perDay + '.') : (d.km + ' km, inside your ' + free + ' km.') };
  }
