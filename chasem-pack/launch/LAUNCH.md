...
Check where you stand at any point:
    ...
    node chasem-landing/tools/launch-check.mjs

---

## 1. Deploy the relay onto chasem.app

The Vercel project `chasem-landing` is the whole thing: the
...
and the webhook, all wired correctly:
    ...
    node chasem-landing/tools/stripe-setup.mjs \
      --key sk_test_YOURKEY \
...
`chasem-landing/db/*.sql`, and a second run does nothing. The photo
...
In `chasem-landing/public/config.js`: `SUPPORT_EMAIL`, `ABN`,
...
In `chasem-app/config.js`: `signup_url`, which the script printed.
...
be added as a sender on the "Chasem" Messaging Service -- scheduled
...
`Chasem <help@chasem.app>`, so the welcome email and every chase email
...
values, and check again:
    ...
    node chasem-landing/tools/launch-check.mjs --relay https://YOUR-PROJECT.vercel.app
