...
File changed: /home/user/portfolio/chasem-landing/api/msg.js (122 -> 167 lines, +51/-6). Nothing else in the repo was touched. Not deployed, not committed.
...
- From name on hosted email is now `"<Trading Name>" <RESEND_FROM address>`; "via Chasem" is gone.
...
## Env vars the owner must set on Vercel (project chasem-landing, Production)
...
- RESEND_FROM = Chasem <hello@yourverifieddomain.com.au>  (must be on a domain verified in Resend; the display name is replaced per painter, the address is kept)
