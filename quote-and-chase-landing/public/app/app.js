...
(function () {
  ...
  function viewQuote(job) {
    ...
    html += '<div class="card"><div class="row"><button class="btn tape" id="pdf">Send the quote</button><button class="btn ghost" id="sharetext">Share a summary</button></div><p class="hint">Makes the PDF and opens your share sheet: text it, WhatsApp it, email it. Follow-ups go in your calendar at ' + (S.follow_up.quote_days || []).join(', ') + ' days.</p>' +
      '<div class="row" id="afterSend" ' + (job.sent_date ? '' : 'hidden') + '><button class="btn sm" id="remind">Add follow-up reminders to calendar</button></div>' +
    ...
    document.getElementById('pdf').addEventListener('click', function () {
      freeze(); try { var d = QCPdf.quotePDF(job, S, priced); QCPdf.deliver(d, job.quote_no + ' ' + (job.client.name || 'quote').replace(/[^\w ]+/g, '') + '.pdf').then(function (how) { toast(how === 'shared' ? 'Sent. Now add the follow-up reminders.' : 'PDF saved to your downloads'); viewQuote(job); }); } catch (e) { toast('PDF failed: ' + e.message); }
    });
  ...
  function viewInvoice(job) {
    ...
    if (job.invoices.length) html += '<div class="card"><h3>Invoices so far</h3><table>' + job.invoices.map(function (i, k) { return '<tr><td>' + esc(i.no) + '<br><span class="hint">' + esc(i.kind) + ', due ' + QCPdf.fmtDate(i.due) + (i.pay_url ? ' · <a href="' + esc(i.pay_url) + '" target="_blank" rel="noopener">card link</a>' : '') + (i.paid_by === 'card' ? ' · paid by card' : '') + '</span></td><td class="n">' + money(i.total) + '</td><td class="n">' + (i.paid_date ? '<span class="pill ok">paid</span>' : '<button class="btn sm" data-paid="' + k + '">Mark paid</button> <button class="btn ghost sm" data-repdf="' + k + '">PDF</button>') + '</td></tr>'; }).join('') + '</table></div>';
    ...
    document.getElementById('mkinv').addEventListener('click', function () {
      ...
      if (S.stripe && S.stripe.enabled && QCStripe.keyLooksRight(S.stripe.key)) { toast('Creating card payment link'); QCStripe.createPaymentLink(S.stripe.key, { amount: inv.total, name: (S.details.trading_name || 'Invoice') + ' ' + inv.no + (job.client.name ? ' for ' + job.client.name : ''), invoiceNo: inv.no, jobId: job.id }).then(function (l) { inv.pay_url = l.url; inv.pay_link_id = l.id; save(); send(); }).catch(function (e) { toast('Card link failed: ' + e.message + '. Invoice sent without it.'); send(); }); } else send();
    });
    var chk = document.getElementById('checkpay'); if (chk) chk.addEventListener('click', function () { var pend = job.invoices.filter(function (i) { return !i.paid_date && i.pay_link_id; }); if (!pend.length) { toast('No card links to check'); return; } toast('Checking'); Promise.all(pend.map(function (i) { return QCStripe.checkPaid(S.stripe.key, i.pay_link_id).then(function (r) { if (r.paid) { i.paid_date = r.when || QCStore.today(); i.paid_by = 'card'; } return r.paid; }); })).then(function (rs) { var np = rs.filter(Boolean).length; if (job.invoices.every(function (x) { return x.paid_date; }) && job.invoices.some(function (x) { return x.kind !== 'deposit'; })) job.status = 'paid'; save(); toast(np ? np + ' paid by card' : 'Nothing paid yet'); viewInvoice(job); }).catch(function (e) { toast('Stripe: ' + e.message); }); });
  ...
  function viewHelp() {
    $app.innerHTML = '<h1>How it works</h1><div class="card"><ol class="steps">' +
      ...
      '<li><span><b>Follow-ups:</b> when you send a quote or invoice, tap once to drop the reminders into your phone calendar at the days you chose in Set-up. Each reminder opens the Follow-ups tab with the message written. You tap Text.</span></li>' +
