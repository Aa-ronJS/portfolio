...
(function () {
  ...
  function chaseText(kind, job, item, days) {
    ...
    if (days <= t1) return { sms: 'Hi ' + f + ', a quick nudge on invoice ' + item.no + ' for ' + amt + ', due ' + due + '. Could you send it through when you get a sec? ' + bank + '. ' + so, subject: 'Invoice ' + item.no + ' (' + amt + ')', email: 'Hi ' + f + ',\n\nJust a quick nudge on invoice ' + item.no + ' for ' + amt + ', which was due ' + due + '. Easy to miss, so no stress. Payment details: ' + bank + '. Reference ' + item.no + '.\n\n' + so };
    if (days <= t2) return { sms: 'Hi ' + f + ', following up on invoice ' + item.no + ' for ' + amt + ', due ' + due + ' and still outstanding. If there is a problem with the work please tell me, otherwise could you pay it this week? ' + bank + '. ' + so, subject: 'Following up: invoice ' + item.no + ' (' + amt + ')', email: 'Hi ' + f + ',\n\nFollowing up on invoice ' + item.no + ' for ' + amt + ', which was due ' + due + ' and is still outstanding. If there is anything about the work holding this up, tell me and I will sort it. Otherwise, could you get it paid this week?\n\nPayment details: ' + bank + '. Reference ' + item.no + '.\n\n' + so };
  ...
  function viewHelp() {
    $app.innerHTML = '<h1>How it works</h1><div class="card"><ol class="steps">' +
      ...
      '<li><span><b>On site.</b> New job, add a room, stick a blank A4 sheet on the wall, photograph the whole wall. Confirm the page outline, confirm the wall outline, save. Next wall.</span></li>' +
      ...
      '<div class="card"><h3>Measuring from a photo</h3><p class="muted">A blank A4 sheet is 210 by 297 mm. The app finds it in the photo and uses it as the ruler, with the wall corners and the lens geometry doing the perspective. Expect one to two percent. No sheet: tap the corners and use a door (2.04 m) or the ceiling height. Typed sizes are estimates and the quote says so.</p></div>' +
